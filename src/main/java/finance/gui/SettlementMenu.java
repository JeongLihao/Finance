...
public final class SettlementMenu extends AbstractContainerMenu {
    ...
    public record DemandRow(UUID id,String commodity,String theme,int quantity,long reward,long deadline,String status,
                            boolean mine,int localPremiumBps,int shortageScore,String pressure,boolean priceReliable){}
    ...
    public static void write(FriendlyByteBuf b,UUID id,String name,String status,int level,int points,List<DemandRow> rows,String message){b.writeUUID(id);b.writeUtf(limit(name,64),64);b.writeUtf(limit(status,24),24);b.writeVarInt(Math.max(0,Math.min(5,level)));b.writeVarInt(Math.max(0,points));int n=Math.min(MAX_ROWS,rows.size());b.writeVarInt(n);for(int i=0;i<n;i++){DemandRow r=rows.get(i);b.writeUUID(r.id());b.writeUtf(limit(r.commodity(),64),64);b.writeUtf(limit(r.theme(),32),32);b.writeVarInt(r.quantity());b.writeLong(r.reward());b.writeLong(r.deadline());b.writeUtf(limit(r.status(),16),16);b.writeBoolean(r.mine());b.writeVarInt(Math.max(7500,Math.min(14000,r.localPremiumBps())));b.writeVarInt(Math.max(0,Math.min(10000,r.shortageScore())));b.writeUtf(limit(r.pressure(),16),16);b.writeBoolean(r.priceReliable());}b.writeUtf(limit(message,96),96);}
    static List<DemandRow> readRows(FriendlyByteBuf b){try{int n=b.readVarInt();if(n<0||n>MAX_ROWS)throw new IllegalArgumentException("settlement rows");List<DemandRow> out=new ArrayList<>(n);for(int i=0;i<n;i++){DemandRow r=new DemandRow(b.readUUID(),b.readUtf(64),b.readUtf(32),b.readVarInt(),b.readLong(),b.readLong(),b.readUtf(16),b.readBoolean(),b.readVarInt(),b.readVarInt(),b.readUtf(16),b.readBoolean());if(r.commodity().isBlank()||r.quantity()<=0||r.reward()<=0||r.deadline()<0||r.localPremiumBps()<7500||r.localPremiumBps()>14000||r.shortageScore()<0||r.shortageScore()>10000||r.pressure().isBlank())throw new IllegalArgumentException("invalid settlement row");out.add(r);}return out;}catch(IndexOutOfBoundsException truncated){throw new IllegalArgumentException("truncated settlement rows",truncated);}}
