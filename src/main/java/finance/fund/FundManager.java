...
public final class FundManager {
    ...
    public static synchronized Result subscribe(UUID player,String fundId,long amount,long day,String operationKey){
        ...
        PlayerFundPosition position=positionInternal(player,id);if(!position.canAdd(shares,amount)||!state.canAddShares(shares))return Result.fail("份额或成本溢出");
        ...
        try{position.add(shares,amount);state.addShares(shares);state.addFees(fee);OPERATION_KEYS.add(key);}catch(ArithmeticException ex){if(fee>0)AccountManager.moveFunds(CentralBank.UUID,definition.custodyAccountId(),fee);AccountManager.moveFunds(definition.custodyAccountId(),player,amount);return Result.fail("基金结算溢出");}
    ...
    public static synchronized Result requestRedemption(UUID player,String fundId,long shares,long day,String operationKey){
        ...
        UUID requestId=UUID.randomUUID();FundRedemptionRequest request=new FundRedemptionRequest(requestId,player,id,shares,day,FundRedemptionRequest.Status.PENDING);REQUESTS.put(requestId,request);OPERATION_KEYS.add(key);
    ...
    private static Result executeRedemption(FundRedemptionRequest request,long day){
        ...
        if(payment<=0||AccountManager.getBalance(definition.custodyAccountId())<gross||!AccountManager.canDeposit(request.playerId(),payment)||!AccountManager.canDeposit(CentralBank.UUID,fee)){request.status(FundRedemptionRequest.Status.PENDING,"等待基金流动性或玩家收款容量",0);return Result.fail("等待流动性");}
    ...
    public static synchronized boolean acknowledgeRisk(UUID player,FundType type){if(player==null||type==null)return false;RISK_ACKNOWLEDGEMENTS.add(player+":"+type.name());EconomySavedData.markDirty();return true;}
