...
public final class InsuranceManager {
    ...
    public static synchronized Result createWarehouseAccident(UUID companyId,String commodity,int quantity,long day,long seed){
        Company c=CompanyManager.getCompany(companyId);MarketPrice price=NpcMarketMaker.getMarketPrice(commodity);int before=c==null?0:c.getInventoryAmount(commodity),lost=Math.min(Math.max(0,quantity),before);
        if(c==null||price==null||lost<=0)return Result.fail("No verifiable inventory loss");UUID id=id("warehouse:"+companyId+":"+commodity+":"+day+":"+seed);if(EVENTS.containsKey(id))return Result.fail("Event already processed");
        long value=cap(BigInteger.valueOf(price.getMidPrice()).multiply(BigInteger.valueOf(lost)));if(!c.removeInventory(commodity,lost))return Result.fail("Inventory loss failed");
        InsuredLossEvent event=new InsuredLossEvent(id,RiskEventType.WAREHOUSE_ACCIDENT,companyId,companyId,day,day,seed,Math.min(10_000,lost*10_000/Math.max(1,before)),commodity,before,lost,price.getMidPrice(),value,0,"authoritative inventory before="+before+" removed="+lost,true);
        EVENTS.put(id,event);autoClaim(event);EconomySavedData.markDirty();return Result.ok(id,"Warehouse accident recorded");
    }
    public static synchronized Result createBusinessInterruption(UUID companyId,int stoppedDays,long actualRevenue,long day,long seed){
        Company c=CompanyManager.getCompany(companyId);if(c==null||stoppedDays<=1||c.getRecentProfits().size()<3)return Result.fail("Insufficient shutdown or profit history");UUID id=id("shutdown:"+companyId+":"+day+":"+seed);if(EVENTS.containsKey(id))return Result.fail("Event already processed");
        long average=(long)c.getRecentProfits().stream().mapToLong(Long::longValue).average().orElse(0);long loss=Math.max(0,cap(BigInteger.valueOf(Math.max(0,average)).multiply(BigInteger.valueOf(stoppedDays)))-Math.max(0,actualRevenue));
        long end;try{end=Math.addExact(day,stoppedDays);}catch(ArithmeticException e){return Result.fail("Event date overflow");}
        InsuredLossEvent event=new InsuredLossEvent(id,RiskEventType.FACILITY_SHUTDOWN,companyId,companyId,day,end,seed,Math.min(10_000,stoppedDays*1_000),"",0,0,0,loss,0,"profit-window="+c.getRecentProfits().size()+" average="+average+" days="+stoppedDays+" actual="+actualRevenue,true);
        EVENTS.put(id,event);autoClaim(event);EconomySavedData.markDirty();return Result.ok(id,"Business interruption recorded");
    }
    public static synchronized Result createLoanDefault(UUID loanId,long recovered,long day){
        CompanyLoan l=CompanyLoanManager.loans().get(loanId);if(l==null||l.status()!=LoanStatus.DEFAULTED||l.lenderType()!=LoanLenderType.COMMERCIAL_BANK)return Result.fail("Loan has no eligible credit loss");UUID id=id("loan-default:"+loanId);if(EVENTS.containsKey(id))return Result.fail("Event already processed");
        long loss=Math.max(0,l.outstandingPrincipal()-Math.max(0,recovered));InsuredLossEvent event=new InsuredLossEvent(id,RiskEventType.LOAN_DEFAULT,l.companyId(),loanId,day,day,0,10_000,"",0,0,0,loss,recovered,"default-principal="+l.outstandingPrincipal()+" recovered="+recovered,true);
