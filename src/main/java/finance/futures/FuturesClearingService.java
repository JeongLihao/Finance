...
public final class FuturesClearingService {
    ...
    public static synchronized List<FuturesSettlementRecord> recentHistory(int limit){if(limit<=0||HISTORY.isEmpty())return List.of();int from=Math.max(0,HISTORY.size()-Math.min(MAX_HISTORY,limit));return List.copyOf(HISTORY.subList(from,HISTORY.size()));}
